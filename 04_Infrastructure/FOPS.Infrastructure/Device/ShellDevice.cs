...
public class ShellDevice : IShellDevice
{
    ...
    {
        // 每次执行时，需要生成shell脚本
        var path = BuildEnvironment.ShellScriptPath + $"fops_{env.BuildId}.sh";
        await File.AppendAllTextAsync(path, shellScript, cancellationToken);

        // 执行脚本
        var exitCode = await ShellTools.Run("/bin/sh", $"-xe {path}", actReceiveOutput, env, env.ProjectSourceDirRoot, cancellationToken);
